                        ...
                        <tbody>
                            {displayLeads.length > 0 ? displayLeads.map((lead, idx) => (
                                <tr key={lead.id || idx} className="border-b border-dark-border last:border-0 hover:bg-white/5 transition-colors">
                                    ...
                                    <td className="px-6 py-4 text-slate-500 max-w-[200px] truncate">{lead.address || '-'}</td>
                                    <td className="px-6 py-4 text-slate-400">⭐ {lead.rating ? Number(lead.rating).toFixed(1) : '–'}</td>
